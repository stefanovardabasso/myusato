...
trait GalrtcDataTable
{
    ...
    public static function dataTableEditColumns($table)
    {
        ...
        $viewAllPermission = Auth::user()->hasPermissionTo('view_all galrtcs');
        $viewOwnPermission = Auth::user()->hasPermissionTo('view_own galrtcs');
        $updateAllPermission = Auth::user()->hasPermissionTo('update_all galrtcs');
        $updateOwnPermission = Auth::user()->hasPermissionTo('update_own galrtcs');
        $deleteAllPermission = Auth::user()->hasPermissionTo('delete_all galrtcs');
        $deleteOwnPermission = Auth::user()->hasPermissionTo('delete_own galrtcs');
    ...
    public static function getDataTableObject($tableId, $route)
    {
        return [
            ...
            'columns' => [
                ...
                [
                    'data' => 'product_id', 'className' => 'dt_col_product_id', 'label' => self::getAttrsTrans('product_id'),
                    'filter' => [ 'type' => "search" ]
                ],
            ],
            'ajax' => [
