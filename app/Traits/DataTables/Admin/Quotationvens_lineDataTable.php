...
trait Quotationvens_lineDataTable
{
    use DataTable;
    ...
    public function scopeDataTableSelectRows($query)
    {
        return $query->select([
            'id'    => 'id',
            'id_quotationven' => 'id_quotationven',
        ]);
    }
    ...
    public static function dataTableEditColumns($table)
    {
        ...
        $viewAllPermission = Auth::user()->hasPermissionTo('view_all quotationvens_lines');
        $viewOwnPermission = Auth::user()->hasPermissionTo('view_own quotationvens_lines');
        $updateAllPermission = Auth::user()->hasPermissionTo('update_all quotationvens_lines');
        $updateOwnPermission = Auth::user()->hasPermissionTo('update_own quotationvens_lines');
        $deleteAllPermission = Auth::user()->hasPermissionTo('delete_all quotationvens_lines');
        $deleteOwnPermission = Auth::user()->hasPermissionTo('delete_own quotationvens_lines');
        ...
        $table->editColumn('actions', function ($row) use($viewAllPermission, $viewOwnPermission, $updateAllPermission, $updateOwnPermission, $deleteAllPermission, $deleteOwnPermission) {
            $routeKey = 'admin.quotationvens_lines';
    ...
    public static function getDataTableObject($tableId, $route)
    {
        return [
            ...
            'columns' => [
                ...
                [
                    'data' => 'id_quotationven', 'className' => 'dt_col_id_quotationven', 'label' => self::getAttrsTrans('id_quotationven'),
                    'filter' => [ 'type' => "search" ]
                ],
            ],
            'ajax' => [
