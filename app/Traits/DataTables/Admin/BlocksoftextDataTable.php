...
trait BlocksoftextDataTable
{
    ...
    public static function dataTableEditColumns($table)
    {
        ...
        $viewAllPermission = Auth::user()->hasPermissionTo('view_all blocksoftexts');
        $viewOwnPermission = Auth::user()->hasPermissionTo('view_own blocksoftexts');
        $updateAllPermission = Auth::user()->hasPermissionTo('update_all blocksoftexts');
        $updateOwnPermission = Auth::user()->hasPermissionTo('update_own blocksoftexts');
        $deleteAllPermission = Auth::user()->hasPermissionTo('delete_all blocksoftexts');
        $deleteOwnPermission = Auth::user()->hasPermissionTo('delete_own blocksoftexts');
    ...
    public static function getDataTableObject($tableId, $route)
    {
        return [
            ...
            'columns' => [
                ...
                [
                    'data' => 'alias', 'className' => 'dt_col_alias', 'label' => self::getAttrsTrans('alias'),
                    'filter' => [ 'type' => "search" ]
                ],
            ],
            'ajax' => [
