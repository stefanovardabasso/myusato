    ...
    <div class="row">
        <div class="col-xs-12">
            <CENTER style="border-radius: 0 50px 50px 0;
                    ...
                    height: 40px;
                    padding: 0 15px;"><strong>{{$offerWithProduct->title}}</strong></CENTER>
            <h5>{{__('Price')}}: {{$offerWithProduct->price}} &euro;</h5>
            <p style="text-align:justify;line-height: 1.4;">{{$offerWithProduct->description}}</p>

{{--<div class="page-break"></div>--}}
            <div style="padding-top: 100px; margin: 0 -100px;">
                <div class="product-image"
                     style="background-image: url({{ url('upload/' . $gallery[0]->name) }});">
            ...
            <table class="table" style="padding-top: 10px;">
                ...
                <tr>
                    ...
                    <td> {{  $offerWithProduct->workingHours }}</td>
                </tr>
            </table>
            <ul style="padding-top: 10%;">
                @foreach($productLines as $lines)
                <li>{{$lines->label}}: {{$lines->answer}}</li>
                @endforeach
            </ul>
{{--            <table class="table">--}}
{{--                <?php $a = 0; ?>--}}
{{--                @for($i=0;$i<count($productLines);$i++)--}}
{{--                    {{$productLines[$i]->label  }}--}}
{{--                    <?php if($a == 0){ ?>--}}
{{--                    <tr style="background-color: #ffcb0f"><?php } ?>--}}
{{--                        <td> {{$productLines[$i]->label  }}</td>--}}
{{--                        <?php if($a == 3){  ?></tr>--}}
{{--                    <tr><?php $a = 0; } ?>--}}
{{--                        <td> {{  $productLines[$i]->answer }}</td>--}}
{{--                        <?php if($a == 3){  ?> </tr><?php }  $a++?>--}}
{{--                @endfor--}}
{{--            </table>--}}
